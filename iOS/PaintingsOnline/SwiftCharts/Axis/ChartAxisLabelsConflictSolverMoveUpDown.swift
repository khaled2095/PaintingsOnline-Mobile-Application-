            ...
            for (_, labelDrawers) in labels {
                ...
                if let (lastDrawer, lastRect) = lastDrawerWithRect {
                    ...
                    if intersection != CGRect.null {
                        ...
                        labelDrawer.screenLoc = ChartPoint(x: labelDrawer.screenLoc.x, y: labelDrawer.screenLoc.y - intersection.height / 2)
                        lastDrawer.screenLoc = ChartPoint(x: lastDrawer.screenLoc.x, y: lastDrawer.screenLoc.y + intersection.height / 2)
                    }
                }
